...
const BLOBS = [
  ...
  { x:.5,  y:.62, r:.44, c:'rgba(180,220,200,', a:.17, ph:2.4, sp:.32 },
  { x:.88, y:.72, r:.32, c:'rgba(160,200,230,', a:.2,  ph:3.6, sp:.38 },
  { x:.18, y:.78, r:.35, c:'rgba(255,160,140,', a:.16, ph:4.8, sp:.3  },
  { x:.42, y:.35, r:.28, c:'rgba(200,230,180,', a:.14, ph:0.6, sp:.42 },
]

export default function WatercolorBg({ className = '' }: { className?: string }) {
    ...
    const draw = () => {
      ...
      BLOBS.forEach(b => {
        ...
        for (let i=0;i<5;i++) {
          ...
          g.addColorStop(0, b.c+(b.a*(1-i*.18))+')')
          g.addColorStop(.55, b.c+(b.a*.4*(1-i*.18))+')')
